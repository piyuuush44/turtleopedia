...
describe('login api test cases', () => {
  ...
  //  console.log(response.body);
    ...
    expect(response.body.result.user.password_hash).toEqual(finalUser.password_hash);
  });
  ...
  //  console.log(finalUser);
    ...
    expect(response.body.error.message).toEqual('Invalid request! User not found');
  });
  it('password incorrect', async () => {
   ...
   // console.log(response.body);
    ...
    expect(response.status).toEqual(200);
    expect(response.body.error.httpStatus).toEqual(400);
    expect(response.body.error.message).toEqual('Invalid request! Wrong Password');
  });
});
